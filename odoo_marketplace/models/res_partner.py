...
class ResPartner(models.Model):
    ...
    def set_to_pending(self):
        if self.seller:
            config_setting_obj = self.env['marketplace.config.settings'].get_default_values()
            if config_setting_obj.get("auto_approve_seller", False):
                self.sudo().approve()
            else:
                self.sudo().state = "pending"
                self.status_msg = self.env["res.config.settings"].mp_config_translatable().get("seller_pending_status_msg", "Thank you for seller request, your request has been already sent for approval we'll process your request as soon as possible.")
