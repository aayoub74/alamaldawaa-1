...
class SellerShop(models.Model):
    ...
    def on_change_seller_wise_settings(self):
        if not self.set_seller_wise_settings:
            self.product_count = self.env['ir.values'].get_default(
                'marketplace.config.settings', 'product_count')
            self.sale_count = self.env['ir.values'].get_default(
                'marketplace.config.settings', 'sale_count')
            self.shipping_address = self.env['ir.values'].get_default(
                'marketplace.config.settings', 'shipping_address')
            self.seller_since = self.env['ir.values'].get_default(
                'marketplace.config.settings', 'seller_since')
            self.seller_t_c = self.env['ir.values'].get_default(
                'marketplace.config.settings', 'seller_t_c')
            self.seller_contact_btn = self.env['ir.values'].get_default(
