...
class account_invoice(models.Model):
    ...
    def create_seller_payment_new(self, sellers_dict):
        for seller_id in sellers_dict.get("seller_ids").keys():
            search_domain = [
                ("payment_type", "=", "cr"), 
        ...
        if sellers_dict:
            ...
            for seller in sellers_dict["seller_ids"].keys():
                ...
                for amount in sellers_dict["seller_ids"][seller]["invoice_line_payment"]:
                    total_amount += amount
                mp_currency_obj = self.env["res.currency"].browse(self.env['ir.values'].get_default('marketplace.config.settings', 'mp_currency_id')) or self.env.user.currency_id
                vals.update({
...
class AccountInvoiceLine(models.Model):
    ...
    seller_commission = fields.Float("Seller Commission", readonly=True)