...
class SellerReview(models.Model):
    ...
    _inherit = ['ir.needaction_mixin', 'website.published.mixin','mail.thread']

    _order = "website_published desc, create_date desc"
    _mail_post_access = 'read'

    # def _website_url(self, cr, uid, ids, field_name, arg, context=None):
    #     res = super(SellerReview, self)._website_url(
    #         cr, uid, ids, field_name, arg, context=context)
    #     for review in self.browse(cr, uid, ids, context=context):
    #         res[review.id] = "/shop/product/%s#website_review_%s" % (
    #             review.marketplace_seller_id.id, review.id)
    #     return res
    ...
    def _get_auto_website_published(self):
        ...
        if config_setting_obj.has_key('review_auto_publish'):
    ...
    rating2 = fields.Integer(compute="_get_rating", string="Rating", copy=False)
